...
MacRadioIo::MacRadioIo(IOPCIDevice *device,IOMemoryMap *mapping){
...
bool MacRadioIo::read32(uint32_t offset,uint32_t &value){
    ...
    __atomic_thread_fence(__ATOMIC_SEQ_CST);return value!=0xffffffff;
}
...
bool MacRadioIo::delayUs(unsigned us){
    if(stopped_||!valid()||us>50000)return false;
    ...
    while(us>=1000){if(stopped_)return false;IOSleep(1);us-=1000;}
    if(us)IODelay(us);return !stopped_;
}
