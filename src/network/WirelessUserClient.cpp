...
void R16WirelessUserClient::stop(IOService *provider){
    if(lock_){IOLockLock(lock_);closed_=true;IOLockUnlock(lock_);}
...
IOReturn R16WirelessUserClient::externalMethod(uint32_t selector,IOExternalMethodArguments *a,
    ...
    if(selector>control::disconnect)return kIOReturnUnsupported;
    if(selector==control::status){
        if(a->structureInputSize||!a->structureOutput||a->structureOutputSize!=sizeof(control::Status))return kIOReturnBadArgument;
        bzero(a->structureOutput,sizeof(control::Status));
    ...
    if(!closed_&&owner_&&!isInactive()){
        ...
        }else result=owner_->disconnectWirelessNetwork();
    }
