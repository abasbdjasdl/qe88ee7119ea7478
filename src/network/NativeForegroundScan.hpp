...
enum class Reason : uint8_t {none,caller,selection,disabled,shutdown,timeout,backend,observation,protocol,clock};
...
public:
    ...
    Admission begin(Readiness r,bool activeScan,uint64_t epoch,uint64_t now,
        ...
        if(activeScan)return Admission::unsupported;
        ...
        status_.plannedChannels=count;status_.phase=Phase::queued;status_.drained=true;dwell_={};
