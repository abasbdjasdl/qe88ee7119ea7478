...
public:
    ...
    bool map() {
        mapping = pci->mapDeviceMemoryWithRegister(kIOPCIConfigBaseAddress2, kIOMapInhibitCache | kIOMapReadOnly);
...
bool RTL8852BEProbe::start(IOService *provider) {
    ...
    ok &= setProperty("DriverVersion", "0.0.3");
    ok &= setProperty("Experiment", "MMIO-SYS-CFG1-01");
    ...
    IOLog("RTL8852BEProbe 0.0.3: %s reads=%u cfg=%08x/%08x command=%04x/%04x/%04x; Wi-Fi unavailable\n",
