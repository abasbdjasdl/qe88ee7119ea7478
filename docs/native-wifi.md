...
## Experimental personal authentication profiles

The kernel selection queue and startup configuration now preserve an explicit WPA1/WPA2 protocol and pairwise/group TKIP or CCMP choice into net80211. Software encryption/decryption remains owned by the existing pinned stack; no key/replay/MIC validation is bypassed. Boot properties accept `R16Security` = `open`, `wpa-psk`, or `wpa2-psk`, and `R16PairwiseCipher`/`R16GroupCipher` = `tkip` or `ccmp`. Defaults preserve the tested WPA2/CCMP configuration. Protected profiles require a prederived 32-byte PMK; open profiles reject a supplied PMK. Unsupported mode names fail instead of falling back. These added combinations are implementation paths awaiting hardware validation, not advertised tested capabilities. The native request adapter remains deliberately narrower until its corresponding IE negotiation support is implemented.

Priority changed at the user's request: complete authentication modes before returning to native UI registration. SAE/OWE are absent from the pinned protocol sources. Enterprise EAP transport, supplicant/certificate integration, PMK handoff and PMF negotiation still need implementation. The initial radio policy continues to restrict transmission to 2.4GHz channels1-11/20MHz; 5GHz and wider channels are not enabled merely because the hardware supports them.
