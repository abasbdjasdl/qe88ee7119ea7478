...
struct Device {
    ...
    unsigned writes=0,fail=0,romError=0,inaccessibleHfcReads=0,inaccessibleClockAccesses=0,gatedHciWrites=0;uint64_t time=0;uint16_t cmd=2;bool stuck=false,noH2c=false,noWde=false,noPle=false,stale=false,loseStop=false,reservedLatched=false,loseClockClear=false,loseMacClear=false,loseMacEnable=false;
    ...
    uint32_t read32(uint32_t a){
        if(a==0x1000&&!(regs[0x8400]&0x40000000))return 0x15f00;
    ...
    bool bootWrite32(uint32_t a,uint32_t v){
        ...
        if((a==0x1000||a==0x1010)&&!(regs[0x8400]&0x40000000)){++gatedHciWrites;return true;}
        if(a==0x8400&&v==0x60440000&&loseMacEnable)return true;
...
int main(){
    ...
    assert(r.initialDmac==0&&r.initialHci==0x15f00&&r.accessReady&&r.hciCaptured&&!d.gatedHciWrites);
    assert(r.accessDmac==0x60440000&&r.accessClock==0x40000&&r.hciBefore==0xc15000);
    ...
    // 0.0.10 hardware: a plausible gated HCI read is not writable. Establish
    // access first, and verify restore before disabling that access again.
    d=Device{};d.bootWrite32(0x1000,0x15700);assert(d.read32(0x1000)==0x15f00&&d.gatedHciWrites==1);
    d=Device{};d.regs[0x8400]=0x60440000;r=b::probe(d);
    assert(r.status==b::Status::ready&&r.cleanupOK&&!d.gatedHciWrites);
    r=b::probe(d);assert(r.status==b::Status::ready&&r.initialDmac==0&&!d.gatedHciWrites);
    d=Device{};d.loseMacEnable=true;r=b::probe(d);
    assert(r.status==b::Status::cleanupFailed&&r.operationStatus==b::Status::writeFailed&&!r.hciCaptured&&!d.gatedHciWrites);
    assert(r.failureAddress==0x8400&&!d.inaccessibleClockAccesses);
    ...
    d=Device{};d.regs[0x8400]=0x60440000;d.bootWrite32(0x1010,d.regs[0x1010]|0x7ff03);
    ...
    for(auto a:d.writeAddresses)assert(a==0x1000||a==0x1010||a==0x8400||a==0x8404);
    ...
    d=Device{};d.noH2c=true;twice=b::probeRepeated(d);assert(!twice.secondAttempted);
    d=Device{};d.loseClockClear=true;twice=b::probeRepeated(d);assert(!twice.secondAttempted);
    for(unsigned i=1;i<=writes;++i){
        d=Device{};d.fail=writes+i;twice=b::probeRepeated(d);
        assert(twice.first.status==b::Status::ready&&twice.secondAttempted&&twice.second.status!=b::Status::ready);
    }
    printf("PASS: cold/warm/repeated MAC-gated HCI access and restore, stop mask, clock cleanup, %u write failures, bounded ROM; no DMA or RF\n",writes);
}
