...
struct Device {
    ...
    unsigned writes=0,fail=0,romError=0,inaccessibleHfcReads=0;uint64_t time=0;uint16_t cmd=2;bool stuck=false,noH2c=false,noWde=false,noPle=false,stale=false,loseStop=false,reservedLatched=false;
...
int main(){
    ...
    printf("PASS: 8852B absent-channel regression, stage-scoped cleanup, clocked HFC checks, DLFW/ROM, %u write failures, bounded timeout; no DMA or RF\n",writes);
}
